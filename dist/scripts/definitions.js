export const items = {
    "copper_ore": {
        fancyName: "Copper Ore",
        info: "",
        type: "ore",
        stackSize: 50,
        atlasCoord: {
            normal: { x: 8, y: 32 },
            small: { x: 8, y: 40 }
        },
        smeltedName: "copper_plate"
    },
    "iron_ore": {
        fancyName: "Iron Ore",
        info: "",
        type: "ore",
        stackSize: 50,
        atlasCoord: {
            normal: { x: 16, y: 32 },
            small: { x: 16, y: 40 }
        },
        smeltedName: "iron_plate"
    },
    "stone_ore": {
        fancyName: "Stone Ore",
        info: "",
        type: "ore",
        stackSize: 50,
        atlasCoord: {
            normal: { x: 0, y: 32 },
            small: { x: 0, y: 40 }
        },
        smeltedName: "stone_brick"
    },
    "coal_ore": {
        fancyName: "Coal Ore",
        info: "",
        type: "ore",
        stackSize: 50,
        atlasCoord: {
            normal: { x: 24, y: 32 },
            small: { x: 24, y: 40 }
        },
        fuelTime: 60 * 15,
    },
    "uranium_ore": {
        fancyName: "Uranium Ore",
        info: "",
        type: "ore",
        stackSize: 50,
        atlasCoord: {
            normal: { x: 32, y: 32 },
            small: { x: 32, y: 40 }
        }
    },
    "copper_plate": {
        fancyName: "Copper Plate",
        info: "Made by smelting copper",
        stackSize: 100,
        type: "ore",
        atlasCoord: {
            normal: { x: 0, y: 0 },
            small: { x: 0, y: 8 }
        },
    },
    "iron_plate": {
        fancyName: "Iron Plate",
        info: "Made by smelting copper",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 8, y: 0 },
            small: { x: 8, y: 8 }
        },
        smeltedName: "steel"
    },
    "stone_brick": {
        fancyName: "Stone Brick",
        info: "Made by smelting stone",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 16, y: 0 },
            small: { x: 16, y: 8 }
        }
    },
    "wood": {
        fancyName: "Wood Planks",
        info: "Get by choping wood",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 24, y: 0 },
            small: { x: 24, y: 8 }
        },
        fuelTime: 60,
    },
    "gear": {
        fancyName: "Gear",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 32, y: 0 },
            small: { x: 32, y: 8 }
        }
    },
    "copper_wire": {
        fancyName: "Copper Wire",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 40, y: 0 },
            small: { x: 40, y: 8 }
        }
    },
    "iron_stick": {
        fancyName: "Iron Stick",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 48, y: 0 },
            small: { x: 48, y: 8 }
        }
    },
    "plastic_bar": {
        fancyName: "Plastic Bar",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 56, y: 0 },
            small: { x: 56, y: 8 }
        }
    },
    "green_circuit": {
        fancyName: "Eletronic Circuit",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 64, y: 0 },
            small: { x: 64, y: 8 }
        }
    },
    "red_circuit": {
        fancyName: "Advanced Circuit",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 72, y: 0 },
            small: { x: 72, y: 8 }
        }
    },
    "blue_circuit": {
        fancyName: "Processing Circuit",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 80, y: 0 },
            small: { x: 80, y: 8 }
        }
    },
    "white_circuit": {
        fancyName: "White Circuit",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 80, y: 0 },
            small: { x: 80, y: 8 }
        }
    },
    "red_cience": {
        fancyName: "Automation Cience Pack",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 96, y: 0 },
            small: { x: 96, y: 8 }
        }
    },
    "green_cience": {
        fancyName: "Logistic Cience Pack",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 104, y: 0 },
            small: { x: 104, y: 8 }
        }
    },
    "blue_cience": {
        fancyName: "Chemical Cience Pack",
        info: "",
        stackSize: 200,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 112, y: 0 },
            small: { x: 112, y: 8 }
        }
    },
    "black_cience": {
        fancyName: "Military Cience Pack",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 120, y: 0 },
            small: { x: 120, y: 8 }
        }
    },
    "white_cience": {
        fancyName: "Space Cience Pack",
        info: "",
        stackSize: 2000,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 0, y: 16 },
            small: { x: 0, y: 24 }
        }
    },
    "steel": {
        fancyName: "Steel Plate",
        info: "",
        stackSize: 100,
        type: "ingredient",
        atlasCoord: {
            normal: { x: 16, y: 16 },
            small: { x: 16, y: 24 }
        }
    },
    "wood_chest": {
        fancyName: "Wooden chest",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 24, y: 16 },
            small: { x: 24, y: 24 }
        }
    },
    "stone_furnace": {
        fancyName: "Stone Furnace",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 56, y: 16 },
            small: { x: 56, y: 24 }
        }
    },
    "steel_furnace": {
        fancyName: "Steel Furnace",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 64, y: 16 },
            small: { x: 64, y: 24 }
        }
    },
    "assembly_machine": {
        fancyName: "Assembly Machine",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 72, y: 16 },
            small: { x: 72, y: 24 }
        }
    },
    "research_lab": {
        fancyName: "Research Lab",
        info: "",
        stackSize: 10,
        type: "placeable",
        atlasCoord: {
            normal: { x: 80, y: 16 },
            small: { x: 80, y: 24 }
        }
    },
    "transport_belt": {
        fancyName: "Transport Belt",
        info: "",
        stackSize: 100,
        type: "placeable",
        atlasCoord: {
            normal: { x: 88, y: 16 },
            small: { x: 88, y: 24 }
        }
    },
    "splitter": {
        fancyName: "Splitter",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 96, y: 16 },
            small: { x: 96, y: 24 }
        }
    },
    "undergroun_belt": {
        fancyName: "Underground Belt",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 104, y: 16 },
            small: { x: 104, y: 24 }
        }
    },
    "burner_mining_drill": {
        fancyName: "Burner Mining Drill",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 112, y: 16 },
            small: { x: 112, y: 16 }
        }
    },
    "inserter": {
        fancyName: "Inserter",
        info: "",
        stackSize: 50,
        type: "placeable",
        atlasCoord: {
            normal: { x: 120, y: 16 },
            small: { x: 120, y: 24 }
        }
    },
};
export const recipes = {
    "gear": { outputQuant: 1, ingredients: [{ name: "iron_plate", quant: 2 }] },
    "copper_wire": { outputQuant: 2, ingredients: [{ name: "copper_plate", quant: 1 }] },
    "iron_stick": { outputQuant: 2, ingredients: [{ name: "iron_plate", quant: 2 }] },
    "green_circuit": { outputQuant: 1, ingredients: [{ name: "copper_wire", quant: 2 }, { name: "iron_plate", quant: 1 }] },
    "red_circuit": { outputQuant: 1, ingredients: [{ name: "copper_wire", quant: 4 }, { name: "green_circuit", quant: 2 }, { name: "plastic_bar", quant: 2 }] },
    "red_cience": { outputQuant: 1, ingredients: [{ name: "copper_plate", quant: 1 }, { name: "gear", quant: 1 }] },
    "green_cience": { outputQuant: 1, ingredients: [{ name: "inserter", quant: 1 }, { name: "transport_belt", quant: 1 }] },
    "wood_chest": { outputQuant: 1, ingredients: [{ name: "wood", quant: 2 }] },
    "stone_furnace": { outputQuant: 1, ingredients: [{ name: "stone_ore", quant: 5 }] },
    "steel_furnace": { outputQuant: 1, ingredients: [{ name: "steel", quant: 6 }, { name: "stone_brick", quant: 10 }] },
    "assembly_machine": { outputQuant: 1, ingredients: [{ name: "green_circuit", quant: 3 }, { name: "gear", quant: 5 }, { name: "iron_plate", quant: 9 }] },
    "research_lab": { outputQuant: 1, ingredients: [{ name: "green_circuit", quant: 10 }, { name: "gear", quant: 10 }, { name: "transport_belt", quant: 4 }] },
    "transport_belt": { outputQuant: 2, ingredients: [{ name: "gear", quant: 1 }, { name: "iron_plate", quant: 1 }] },
    "splitter": { outputQuant: 1, ingredients: [{ name: "green_circuit", quant: 5 }, { name: "iron_plate", quant: 5 }, { name: "transport_belt", quant: 4 }] },
    "inserter": { outputQuant: 1, ingredients: [{ name: "green_circuit", quant: 1 }, { name: "iron_plate", quant: 1 }, { name: "gear", quant: 1 }] },
    "burner_mining_drill": { outputQuant: 1, ingredients: [{ name: "gear", quant: 3 }, { name: "iron_plate", quant: 3 }, { name: "stone_furnace", quant: 1 }] },
    "underground_belt": { outputQuant: 1, ingredients: [{ name: "iron_plate", quant: 10 }, { name: "transport_belt", quant: 5 }] },
};
export const entities = {
    "wood_chest": {
        atlasCoord: { x: 24, y: 16 },
        sizeInPixels: { w: 8, h: 8 },
        sizeInTiles: { w: 1, h: 1 }
    },
    "stone_furnace": {
        atlasCoord: { x: 72, y: 112 },
        sizeInPixels: { w: 16, h: 16 },
        sizeInTiles: { w: 2, h: 2 }
    },
    "steel_furnace": {
        atlasCoord: { x: 88, y: 112 },
        sizeInPixels: { w: 16, h: 16 },
        sizeInTiles: { w: 2, h: 2 }
    },
    "assembly_machine": {
        atlasCoord: { x: 24, y: 104 },
        sizeInPixels: { w: 24, h: 24 },
        sizeInTiles: { w: 3, h: 3 }
    },
    "burner_mining_drill": {
        atlasCoord: { x: 0, y: 88 },
        sizeInPixels: { w: 16, h: 16 },
        sizeInTiles: { w: 2, h: 2 }
    },
    "research_lab": {
        atlasCoord: { x: 0, y: 104 },
        sizeInPixels: { w: 24, h: 24 },
        sizeInTiles: { w: 3, h: 3 }
    },
};
